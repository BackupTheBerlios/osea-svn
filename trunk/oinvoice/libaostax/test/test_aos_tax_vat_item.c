...
#define MODULE_TEST_NAME "aos_tax_vat_item"
...
gboolean print_vat_item (gpointer key, gpointer value, gpointer data)
{
	AosTaxVatItem *item = (AosTaxVatItem *) value;
	

	printf("%d\t%s\t%s\t%s\n", item->id, item->reference, item->name, item->description );
...
gboolean test1_user_function (OseaClientData * data, gpointer user_data)
{
	...
	if (data->state == OSEACLIENT_OK) {
		oseaclient_list_foreach (data->data, print_vat_item, NULL);
		test1_exit_var = TRUE;
	}else {
...
// List item
	...
	if (!aos_tax_vat_item_list ( 0, 0, test1_user_function, NULL)) {
		return FALSE;
	}
...
// New item	
	...
	if (!aos_tax_vat_item_new ("ref", "name", "description", 
...
// Remove item	
	test3_exit_var = FALSE;
	if (!aos_tax_vat_item_remove (test3_identifier, test3_user_function2, NULL)) {
		return FALSE;
	} 
	while (test3_exit_var == FALSE) {
		sleep (1);
...
// New item	
	test3_exit_var = FALSE;
	if (!aos_tax_vat_item_new ("ref_2", "name_2", "description_2", 
					 test3_user_function, NULL)) {
		return FALSE;
	}
	while (test3_exit_var == FALSE) {
...
// Remove item	
	test3_exit_var = FALSE;
	if (!aos_tax_vat_item_remove (identifier, test3_user_function2, NULL)) {
		return FALSE;
	} 
	while (test3_exit_var == FALSE) {
		sleep (1);
...
// Edit item
	test3_exit_var = FALSE;
	if (!aos_tax_vat_item_edit (identifier, "new_ref", "new_name", "new_description", 
...
// Edit item
	test3_exit_var = FALSE;
	if (!aos_tax_vat_item_edit (identifier-3, "new_ref", "new_name", "new_description", 
					  test3_user_function2, NULL)) {
		return FALSE;
	}
	while (test3_exit_var == FALSE) {
...
TestNode test_suite []  = { {"aos_tax_vat_item_list/new/remove LVL 1", test1},
			    {"aos_tax_vat_item_list/new/remove LVL 2", test2},
			    {"aos_tax_vat_item_list/new/remove/edit LVL 3", test3},
			    {NULL, NULL}};
