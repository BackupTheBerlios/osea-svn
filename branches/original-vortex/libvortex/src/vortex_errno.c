...
void    vortex_errno_show_error        (gint __errno)
{

#ifdef G_OS_WIN32
	switch (errno) {
	case WSAEINTR:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEINTR);
	case WSAEACCES:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEACCES);
	case WSAEFAULT:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEFAULT);
	case WSAEINVAL:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEINVAL);
	case WSAEMFILE:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEFAULT);
	case WSAEWOULDBLOCK:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEWOULDBLOCK);
	case WSAEINPROGRESS:        
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEINPROGRESS);
	case WSAEALREADY:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEALREADY);           
	case WSAENOTSOCK:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAENOTSOCK);
	case WSAEDESTADDRREQ:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEDESTADDRREQ);
	case WSAEMSGSIZE:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEMSGSIZE);           
	case WSAEPROTOTYPE:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEPROTOTYPE);
	case WSAENOPROTOOPT:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAENOPROTOOPT);
	case WSAEPROTONOSUPPORT:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEPROTONOSUPPORT);
	case WSAESOCKTNOSUPPORT:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAESOCKTNOSUPPORT);
	case WSAEOPNOTSUPP:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEOPNOTSUPP);
	case WSAEPFNOSUPPORT:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEPFNOSUPPORT);
	case WSAEAFNOSUPPORT:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEAFNOSUPPORT);
	case WSAEADDRINUSE:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEADDRINUSE);
	case WSAEADDRNOTAVAIL:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEADDRNOTAVAIL);
	case WSAENETDOWN:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAENETDOWN);
	case WSAENETUNREACH:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAENETUNREACH);
	case WSAENETRESET:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAENETRESET);
	case WSAECONNABORTED:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAECONNABORTED);
	case WSAECONNRESET:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAECONNRESET);
	case WSAENOBUFS:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAENOBUFS);
	case WSAEISCONN:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEISCONN);
	case WSAENOTCONN:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAENOTCONN);
	case WSAESHUTDOWN:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAESHUTDOWN);
	case WSAETIMEDOUT:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAETIMEDOUT);
	case WSAECONNREFUSED:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAECONNREFUSED);
	case WSAEHOSTDOWN:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEHOSTDOWN);
	case WSAEHOSTUNREACH:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEHOSTUNREACH);
	case WSAEPROCLIM:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEPROCLIM);
	case WSASYSNOTREADY:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSASYSNOTREADY);
	case WSAVERNOTSUPPORTED:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAVERNOTSUPPORTED);
	case WSANOTINITIALISED:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSANOTINITIALISED);
	case WSAEDISCON:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAEDISCON);
	case WSATYPE_NOT_FOUND:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSATYPE_NOT_FOUND);     
	case WSAHOST_NOT_FOUND:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSAHOST_NOT_FOUND);
	case WSATRY_AGAIN:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSATRY_AGAIN);
	case WSANO_RECOVERY:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSANO_RECOVERY);
	case WSANO_DATA:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSANO_DATA);            
	case WSA_INVALID_HANDLE:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSA_INVALID_HANDLE);
	case WSA_INVALID_PARAMETER:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSA_INVALID_PARAMETER);
	case WSA_IO_INCOMPLETE:     
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSA_IO_INCOMPLETE);
	case WSA_IO_PENDING:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSA_IO_PENDING);
	case WSA_NOT_ENOUGH_MEMORY:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSA_NOT_ENOUGH_MEMORY);
	case WSA_OPERATION_ABORTED:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSA_OPERATION_ABORTED);
	case WSASYSCALLFAILURE:
		g_log (LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  ERRNO_WSASYSCALLFAILURE);
	}
#else
	// fill up with GNU/Linux case
#endif
	return;
}
...
gchar * vortex_errno_get_error         (gint __errno)
{
#ifdef G_OS_WIN32
	switch (errno) {
	case WSAEINTR:
...
gchar * vortex_errno_get_last_error    ()
{
