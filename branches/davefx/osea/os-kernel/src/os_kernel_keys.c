...
gchar *  os_kernel_keys_generate (gchar * user, gchar * satellite_server)
{
	...
	dataset = afgs_command_execute_single_query ("SELECT p.name  \
	...
	height = coyote_dataset_get_height (dataset);
	for (i = 0; i < height; i++) {
		if ((i + 1) == height)
			g_string_append_printf (result_key, "%s", coyote_dataset_get (dataset, i, 0));
		else
			g_string_append_printf (result_key, "%s,", coyote_dataset_get (dataset, i, 0));
...
gboolean os_kernel_keys_request (CoyoteXmlServiceData *data, gpointer user_data, RRChannel * channel, gint msg_no)
{
	GList          * values;
	CoyoteDataSet  * af_key_dataset;
	RRConnection   * rr_connection;
	gchar          * new_af_key = NULL;
	AfgsAfKey      * channel_af_key = NULL;
	

	// Check for correct params
	values = afgs_message_check_params (data, "session_id","server_name", NULL);
	if (!values) {
	...
	channel_af_key = afgs_afkeys_get_connection_key  (rr_connection);

	if (! channel_af_key) {
		/* Although there's no permissions for this service, you must
		   set an af-key before using it (you need to login before). */
		afgs_log_write ("Error. Trying to refresh an af-key before logging in.");
	...
	if (! os_kernel_session_is_current (g_list_nth_data (values, 0))) {
		afgs_log_write ("Refresh af-key. Permission denied: session %s expired.",
				g_list_nth_data (values, 0));
		afgs_message_error_answer (channel, msg_no, "Session expired",
					   COYOTE_CODE_SESSION_EXPIRED);
		return FALSE;
	}

 	afgs_log_write ("Refreshing af-key for session_id %s and server %s", g_list_nth_data (values, 0),
	...
	new_af_key = os_kernel_keys_generate (channel_af_key->user, g_list_nth_data (values, 1));
 	...
 	af_key_dataset = coyote_dataset_new (); 

 	coyote_dataset_add (af_key_dataset, new_af_key); 

	afgs_message_ok_answer (channel, msg_no, "Af-key refreshed", COYOTE_CODE_OK,
				af_key_dataset, NULL);
